...
export default function InvoiceList() {
  ...
  const filterInvoices = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (inv) =>
          inv.extracted_data?.supplier_name
            ?.toLowerCase()
            .includes(searchTerm.toLowerCase()) ||
          inv.extracted_data?.buyer_name
            ?.toLowerCase()
            .includes(searchTerm.toLowerCase()) ||
          inv.extracted_data?.invoice_no
            ?.toLowerCase()
            .includes(searchTerm.toLowerCase()) ||
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      toast.success(
        `Exported ${selectedInvoices.length} invoices as ${format.toUpperCase()}`,
      );
      setSelectedInvoices([]);
    } catch (error) {
    ...
    <Layout>
      <div className="space-y-6" data-testid="invoice-list-page">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1
              className="text-3xl font-manrope font-bold text-[#0B2B5C]"
              data-testid="invoice-list-title"
            >
              All Invoices
            </h1>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col md:flex-row gap-4 items-end">
                <div className="flex-1">
                  <Label
                    htmlFor="start-date"
                    className="text-sm font-medium text-[#0B2B5C] mb-1 flex items-center gap-2"
                  >
                    <Calendar size={16} />
                    From Date
                  </Label>
            ...
            {loading ? (
              ...
              <div className="p-12">
                <Alert data-testid="no-invoices-found-message">
                  <AlertDescription>
                    No invoices found. Try adjusting your filters or upload a
                    new invoice.
                  </AlertDescription>
                </Alert>
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/50">
                      ...
                      <TableHead className="font-bold text-[#0B2B5C]">
                        Type
                      </TableHead>
                      <TableHead className="font-bold text-[#0B2B5C]">
                        Invoice No
                      ...
                      <TableHead className="font-bold text-[#0B2B5C] text-right">
                        Amount
                      </TableHead>
                      <TableHead className="font-bold text-[#0B2B5C] text-center">
                        Confidence
                      </TableHead>
                      <TableHead className="font-bold text-[#0B2B5C] text-center">
                        Status
                  ...
                  <TableBody>
                    {filteredInvoices.map((invoice) => {
                      const avgConfidence =
                        Object.values(invoice.confidence_scores).reduce(
                          (a, b) => a + b,
                          0,
                        ) / 8;
                      return (
                        ...
                        >
                            ...
                            <Badge
                              ...
                              className={
                                invoice.invoice_type === "sales"
                                  ? "bg-[#10B981]/10 text-[#10B981] border-[#10B981] font-semibold"
                                  : "bg-[#EF4444]/10 text-[#EF4444] border-[#EF4444] font-semibold"
                              }
                          ...
                          <TableCell className="text-center">
                            <span
                              className={`font-mono font-medium ${getConfidenceColor(avgConfidence)}`}
                            >
                          ...
                          <TableCell className="text-center">
                            <Badge
                              ...
                              data-testid={`status-badge-${invoice.id}`}
                            >
                              {invoice.status}
                            </Badge>
                          </TableCell>
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  data-testid={`actions-menu-${invoice.id}`}
                                >
