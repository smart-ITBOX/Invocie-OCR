...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const totalAmount = invoices.reduce(
        (sum, inv) => sum + (inv.extracted_data?.total_amount || 0),
        0,
      );
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "exported":
        return "text-[#0B2B5C] bg-[#0B2B5C]/10";
      default:
    ...
    <Layout>
      <div className="space-y-8" data-testid="dashboard-page">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1
              className="text-3xl font-manrope font-bold text-[#0B2B5C] mb-2"
              data-testid="dashboard-title"
            >
              Invoice Processing Dashboard
            </h1>
            <p className="text-muted-foreground">
              Upload, extract, and verify invoice data with AI
            </p>
          </div>
          <Button
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
          <Card className="border-[#0B2B5C]/10 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <CardContent>
              <div
                className="text-2xl font-mono font-bold text-[#0B2B5C]"
          ...
          <Card className="border-[#EF4444]/10 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <CardContent>
              <div
                ...
                data-testid="purchase-invoices-count"
              >
          ...
          <Card className="border-[#10B981]/10 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <CardContent>
              <div
                className="text-2xl font-mono font-bold text-[#10B981]"
                data-testid="sales-invoices-count"
              >
                {stats.salesInvoices}
              </div>
            </CardContent>
          ...
          <Card className="border-[#F59E0B]/10 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <CardContent>
              <div
                className="text-2xl font-mono font-bold text-[#F59E0B]"
                data-testid="pending-invoices-count"
              >
                {stats.pending}
              </div>
            </CardContent>
          ...
          <Card className="border-[#10B981]/10 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <CardContent>
              <div
                className="text-2xl font-mono font-bold text-[#10B981]"
                data-testid="verified-invoices-count"
              >
                {stats.verified}
              </div>
            </CardContent>
        ...
        <Card className="border-[#FFD700]/20 shadow-lg">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xl font-manrope text-[#0B2B5C]">
                  Upload Invoices
                </CardTitle>
                <CardDescription>
                  Single or batch upload (max 20 files) - PDF, JPG, PNG
                </CardDescription>
              </div>
              <Tabs
                value={invoiceType}
          ...
          <CardContent>
                ...
                ${
                  isDragActive
                    ...
                    : invoiceType === "purchase"
                      ? "border-[#FFD700] bg-[#FFD700]/5"
                      : "border-green-300 bg-green-300/5 hover:border-[#FFD700] hover:bg-[#FFD700]/5"
                }
            ...
            >
              <input {...getInputProps()} />
              <Upload
                className={`${invoiceType === "purchase" ? "text-[#FFD700]" : "text-green-300"} mx-auto mb-4`}
                size={48}
              />
              {uploading ? (
                <div className="space-y-3">
                  <p className="text-[#0B2B5C] font-medium">
                    Please wait while we processing invoice...
                  </p>
                  {uploadProgress > 0 && (
                    <Progress
                ...
                <p
                  className={`${invoiceType === "purchase" ? "text-[#FFD700]" : "text-green-300"} font-medium`}
                >
                  Drop invoices here
                </p>
              ) : (
                <>
                  <p
                    className={`${invoiceType === "purchase" ? "text-[#FFD700]" : "text-green-300"} font-medium mb-2`}
                  >
                    Drop {invoiceType === "purchase" ? "purchase" : "sales"}{" "}
                    invoices here or click to browse
                  </p>
                  <p
                    className={`${invoiceType === "purchase" ? "text-[#7a6801]" : "text-green-700"} text-sm text-muted-foreground`}
                  >
                    Upload 1-20 invoices at once (Max 10MB each)
                  </p>
                </>
              )}
            </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <div className="font-mono font-medium text-[#0B2B5C]">
                          ₹
                          {(
                            invoice.extracted_data?.total_amount || 0
                      ...
                      <span
                        className={`px-3 py-1 rounded-sm text-xs font-medium ${getStatusColor(invoice.status)}`}
                      >
                        {invoice.status}
                      </span>
                    </div>
